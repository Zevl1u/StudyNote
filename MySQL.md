...
记录了所有执行时间超过指定参数（long_query_time，以秒为单位，默认10秒）的所有SQL语句的日志。

```sql
SHOW VARIABLES LIKE 'slow_query_log';
```

默认不开启(OFF)，需要再配置文件（/etc/my.cnf）中配置如下信息：

```
slow_query_log=1 # 开启慢查询日志
long_query_time=2 # 设置超过2秒就记录
```

配置完毕后，重启MySQL服务器测试，查看慢日志文件中记录的信息/var/lib/mysql/localhost-slow.log
...
默认关闭，可以通过set语句再session/global级别开启profiling
...
SHOW PROFILE CPU FOR QUERY query_id; # 查看指定query_id的SQL语句各阶段CPU情况
```

#### EXPLAIN

在语句之前加上EXPLAIN或者DESC命令获取MySQL如何执行SELECT语句信息，包括在语句执行过程中如何连接和连接的顺序。

- id：SELECT查询的序列号，表示查询中执行SELECT子句或者操作表的顺序。**id相同，执行顺序从上到下；id不同，值越大越先执行。**
- type：表示连接类型，性能好->差分别是：NULL、system、const、eq_ref、ref、range、index、all。优化手段就是让连接类型尽量往const优化。一般根据主键或者唯一索引去访问，类型是const；非唯一索引的话会是ref。index表示用了索引，但是扫描了全部索引；all表示全表扫描。
- key：实际使用的索引，若为NULL，则未使用索引
- filtered：返回结果占需读取行数的占比，越大越好
...
- 满足业务需求的情况下，尽量降低主键的长度（减少磁盘io）
- 插入数据时，尽量选择顺序插入，选择使用 AUTO_INCREMENT 自增主键（乱序容易出现页分裂，效率低）
- 尽量不要使用 UUID 做主键或者是其他的自然主键，如身份证号（UUID可能无线，身份证号比较长）
- 业务操作时，避免对主键的修改（改主键，还要动二级索引，效率低）
...
通俗的说，视图只保存了查询的SQL逻辑，不保存查询结果。所以在创建视图的时候，主要工作就落在创建这条SQL查询语句上。

- 创建

  `CREATE [OR REPLLACE] VIEW 视图名称[(列表名称)] AS SELCET语句 [WITH [CASCADED | LOCAL] CHECK OPTION]`
...
- 修改
  ...
  方式一：`CREATE [OR REPLACE] VIEW 视图名称[(视图列表)] AS SELECT语句 [WITH [CASCADED | LOCAL] CHECK OPTION]`

  方式二：`ALTER VIEW 视图名称[(视图列表)] AS SELECT语句 [WITH [CASCADED | LOCAL] CHECK OPTION]`
...
当使用WITH CHECK OPTION子句创建视图时，MySQL会通过视图检查正在更改的每个行，例如插入，更新，删除，以使其符合视图定义。MySQL允许基于另一个视图创建视图，他还会检查依赖视图中的规则以保持一致性。检查范围提供了两个选项：cascaded和local，默认值为cascaded(级联)

- cascaded：会自动检查依赖的表，相当于给所有依赖的表都加上了检查，强制要求依赖的表也要满足条件

- local：若依赖的表要求检查则会检查，不要求检查则不检查
...
- 安全：数据库可以授权，但不能授权到数据库特定的行列上。通过视图用户只能查询和修改他们所能见到的数据
- 数据独立：视图可帮助用户屏蔽真实表结构变化带来的影响

### 存储过程

指事先经过编译并存储在数据库中的一段SQL语句的集合，调用存储过程可以简化应用开发人员的工作，减少数据库和应用服务器之间的传输，对于提高数据处理效率是有益的。

实际上就是对数据库SQL语言层面的代码封装与重用，类似定义了一个函数。
...
```sql
-- 查询指定数据的存储过程及状态信息 xxx是数据库名
SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='xxx';
-- 查询某个存储过程的定义
SHOW CREATE PROCEDURE 存储过程明;
```
...
- 系统变量：由MySQL服务器提供，非用户自定义，属于服务器层面。分为全局变量(GLOBAL)和会话变量(SESSION)
  ...
  - 设置系统变量
    ...
    注意设置完GLOBAL变量后，服务器重启之后会恢复成初始状态。要想不失效要在/etc/my.cnf中配置。

- 用户自定义变量：用户自定义的变量，不用提前声明，用的时候直接`@变量名`使用就可以。作用域为当前会话。

  - 赋值

    ```sql
    SET @var_name=expr[,@varname=expr]...;
    -- 推荐用:=  用于和MySQL里的比较运算符=区别
    SET @var_name:=expr[,@varname=expr]...;
    
    SELECT @var_name:=expr[,@varname=expr]...;
    -- 将从表中的查询结果赋值给变量
...
- 局部变量：在局部定义生效的变量，访问之前需要DECLARE声明。可用作存储过程内的局部变量和输入参数，局部变量的作用范围是在声明所在的BEGIN..END语句块。
